...
```
           pt_alloc            creation done
  UNUSED ----------> EMBRYO ----------------> RUNNABLE
    ^                                           |   ^
    | parent reaps                    scheduler |   | quantum expires,
    | the corpse                     dispatches |   | or wakeup
    | (wait)                                    v   |
  ZOMBIE <------------- exit ---------------- RUNNING
                                                |
                                     blocks on: v
                            +---------+-----------+----------+
                            | SENDING | RECEIVING | SLEEPING |
                            +----+----+-----+-----+-----+----+
                         partner |  message |      timer |
                        receives |  arrives |      fires |
                                 v          v            v
                                    (back to RUNNABLE)
```
