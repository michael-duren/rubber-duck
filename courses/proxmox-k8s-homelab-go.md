...
func TestCheckpointClaimed(t *testing.T) {
	if !DONE {
		t.Fatal("flip DONE to true once the blueprint repo exists")
	}
}
```
...
func TestCheckpointClaimed(t *testing.T) {
	if !DONE {
		t.Fatal("flip DONE to true once the node is up and updated")
	}
}
```
...
func TestCheckpointClaimed(t *testing.T) {
	if !DONE {
		t.Fatal("flip DONE to true once curl returns your node list")
	}
}
```
...
func TestCheckpointClaimed(t *testing.T) {
	if !DONE {
		t.Fatal("flip DONE to true once apply created the pool")
	}
}
```
...
func TestCheckpointClaimed(t *testing.T) {
	if !DONE {
		t.Fatal("flip DONE to true once the VM survived a rebuild")
	}
}
```
...
func TestCheckpointClaimed(t *testing.T) {
	if !DONE {
		t.Fatal("flip DONE to true once baseline.yml is idempotent")
	}
}
```
...
func TestRebuiltFromCode(t *testing.T) {
	if !RebuiltFromCode {
		t.Fatal("flip RebuiltFromCode to true once the VM came back from the repo")
	}
}

func TestBaselineIdempotent(t *testing.T) {
	if !BaselineIdempotent {
		t.Fatal("flip BaselineIdempotent to true once a second baseline run is all ok")
	}
}

func TestRepoIsTheLab(t *testing.T) {
	if !RepoIsTheLab {
		t.Fatal("flip RepoIsTheLab to true once the repo is committed and self-explanatory")
	}
}
```
...
func TestCheckpointClaimed(t *testing.T) {
	if !DONE {
		t.Fatal("flip DONE to true once you can say it back")
	}
}
```
...
func TestCheckpointClaimed(t *testing.T) {
	if !DONE {
		t.Fatal("flip DONE to true once all three VMs pong")
	}
}
```
...
func TestCheckpointClaimed(t *testing.T) {
	if !DONE {
		t.Fatal("flip DONE to true once kubectl sees the node")
	}
}
```
...
func TestCheckpointClaimed(t *testing.T) {
	if !DONE {
		t.Fatal("flip DONE to true once three nodes are Ready")
	}
}
```
...
func TestCheckpointClaimed(t *testing.T) {
	if !DONE {
		t.Fatal("flip DONE to true once whoami answers by hostname")
	}
}
```
...
func TestAppSurvivedNodeLoss(t *testing.T) {
	if !AppSurvivedNodeLoss {
		t.Fatal("flip AppSurvivedNodeLoss to true once the app rode out the node loss")
	}
}

func TestWorkerRebuiltAndRejoined(t *testing.T) {
	if !WorkerRebuiltAndRejoined {
		t.Fatal("flip WorkerRebuiltAndRejoined to true once the rebuilt VM rejoined Ready")
	}
}

func TestClusterRestoredFromRepo(t *testing.T) {
	if !ClusterRestoredFromRepo {
		t.Fatal("flip ClusterRestoredFromRepo to true once the cluster is back to full strength")
	}
}
```
