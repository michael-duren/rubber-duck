...
Click through `put("dot", 7)` landing on a chained bucket — walk the whole chain first (no match), then prepend at the head:
...
steps: {
  'hash("dot") = 0x9c…4a — mod 6 → bucket 2': {
    (buckets."2" -> e3)[0].style.stroke: "#d97706"
    (buckets."2" -> e3)[0].style.stroke-width: 3
  }
