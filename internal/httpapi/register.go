...
type CourseStore interface {
	...
	// Learning paths: ordered tracks of courses. Reads only — path writes
	// go through the proposal workflow (see proposals.go) or `duckserver
	// seed`. ListPathSources feeds /api/v1/export so the repo's paths/
	// mirror stays in sync like courses/.
	...
	ListPathSources(ctx context.Context) ([]domain.PathExport, error)
}

// ProposalStore is the slice of the store the proposal API needs. Reviewing
// and publishing are web-only flows; the CLI only authors. kind is
// domain.KindCourse or domain.KindPath (path proposals put the path slug
// in the slug argument and pass language "").
...
func Register(mux *http.ServeMux, logger *slog.Logger, users UserStore, store CourseStore, proposals ProposalStore) {
	...
	// Learning paths: reads are public like courses; writes go through the
	// same proposal workflow (a "path:" frontmatter document POSTed to
	// /api/v1/proposals), or `duckserver seed` for bootstrap imports.
